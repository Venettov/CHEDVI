...
def clean_dataframe_columns(df):
    """
    Standardizes column names and maps Camden headers to database keys.
    Supports both raw spreadsheets and exported CSVs.
    """
    # 1. Standard cleaning
    df.columns = df.columns.str.strip().str.lower().str.replace(' ', '_').str.replace('%', '').str.replace('$', '')
    
    # 2. Exhaustive Mapping (CSV Cleaned Header -> Database Model Field)
    column_map = {
        'latitude': 'latitude',
        'longitude': 'longitude',
...
def reload_database_from_csv():
    try:
        if not os.path.exists(DATA_FILE): return False, "Data file not found."
        ...
        for _, row in df.iterrows():
            neighborhood = NeighborhoodHealth(
                ...
                # Wrapped in clean_numeric to ensure strings convert to floats correctly
